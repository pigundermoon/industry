...
namespace ZQ
{
	namespace ZQ_ImageIO
	{
        ...
        template<class T>
        bool loadMat(ZQ_DImage<T>& im, cv::Mat_<unsigned short> img);
...
//		IplImage* MakeColorWheelImage1(int N, bool user_input, float max_rad);
        ...
        template<class T>
        bool ZQ_ImageIO::loadMat(ZQ_DImage<T>& im, cv::Mat_<unsigned short> img)
        {
            ...
            for(int i = 0;i < height;i++)
            {
                for(int j = 0;j < width;j++)
                {
                        im_Data[i*width+j] = double(img(i,j))/65535.0;
                }
            }
